...
ISR(ADC_vect)
{
    ...
    static const uint8_t sensorMap[PI_TOTAL_SENSORS] = { 0, 1, 2, 3, 4, 6, 7, 7 };
    ...
    if (initSensor)
    {
        ...
        if(currentSensor == (PI_TOTAL_SENSORS - 1))
		{
			PORTC &= ~(1<<5);
		}
		else if(currentSensor == PI_TOTAL_SENSORS)
        {
            PORTC |= (1<<5);
			currentSensor = 0;
